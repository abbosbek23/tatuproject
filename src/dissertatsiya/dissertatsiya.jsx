...
const Dissertatsiya = () => {
  return (
    <Box display={"flex"} sx={{height:"80vh",paddingInline:"120px"}}>
        {
            dissertatsiyalar.map((item)=><Box key={item.id} sx={{width:"400px",border:"1px solid #B5B5B5",height:"420px"}}>
               <img src={item.image} style={{marginLeft:"30%"}} width={"40%"} alt="dissertatsiya" />
               ...
               <Typography sx={{fontSize:"16px"}}><span>Dissertasiya ishini yakunlash vaqti</span> {item.deadline}</Typography>
            </Box>)
        }
    </Box>
  )
